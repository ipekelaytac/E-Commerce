...
@section('content')
    <div class="pageheader">
        <div class="row pd-t-5 pd-b-25 justify-content-between pd-x-20">
            <div class="pull-left"><h1 class="pd-0 mg-0 tx-20 text-overflow">Markalar</h1></div>
            <div class="pull-right">
                <a href="{{ route('management.brand.add') }}" class="btn btn-success">Yeni Ekle</a>
            </div>
        </div>
    </div>
                            ...
                            @foreach ($list as $entry)
                                <tr>
                                    <td class="align-middle" style="width: 75px">
                                        <img class="wd-50" class="img-responsive"  src="{{ $entry->brand_image!=null ? asset('uploads/brands/' . $entry->brand_image) : 'https://via.placeholder.com/200?text=UrunResmi' }}" alt="">
                                    </td>
                                    <td class="align-middle">{{ $entry->id }}</td>
                                    <td class="align-middle">{{ $entry->brand_name }}</td>
                                    <td class="tx-center">
                                        <div class="dropdown">
                                            <a href="" data-toggle="dropdown" aria-haspopup="true"
                                               aria-expanded="false"><i class="icon-options"></i></a>
                                            <ul class="dropdown-menu dropdown-menu-right">
                                                <li class="dropdown-item"><a class="dropdown-link tx-13 tx-gray-500" href="{{ route('management.brand.delete', $entry->id) }}"><i class="icon-close mr-2"></i>Sil</a></li>
                                                <li class="dropdown-item"><a class="dropdown-link tx-13 tx-gray-500" href="{{ route('management.brand.update', $entry->id) }}"><i class="icon-note mr-2"></i>Düzenle</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
